...
def leapfrog (particle,center_arr,t=0,out_count=0,testflag=False):
    ...
    a_mid = np.transpose(np.transpose(particle_mid[:,7:10])/particle_mid[:,0])/scale_factor(t*step_time)**3
    ...
    while t<=N_t:
        ...
        a_mid = np.transpose(np.transpose(particle_mid[:,7:10])/particle_mid[:,0])/scale_factor((t-0.5)*step_time)**3
        
        # Forward step of main step using mid step
        particle2[:,1:4] += step_time*particle_mid[:,4:7]/scale_factor((t-0.5)*step_time)
        particle2[:,1:4] %= dl
        ...
        a = np.transpose(np.transpose(particle2[:,7:10])/particle2[:,0])/scale_factor(t*step_time)**3
    
        # Forward step of mid step using main step
        particle_mid[:,1:4] += step_time*particle2[:,4:7]/scale_factor(t*step_time)
        particle_mid[:,1:4] %= dl
